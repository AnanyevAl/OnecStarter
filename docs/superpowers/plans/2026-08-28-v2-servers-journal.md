...
> Правка вслед за находкой (28.08.2026, исполнитель задачи 1, проверено
> экспериментом на машине): Job НЕ поглощает уже существующих потомков —
> в Job попадают только дети, порождённые ПОСЛЕ assign родителя. Тест
> Step 1 в исходной редакции ниже назначал Job после рождения внука
> и падал на корректной реализации; в коде тест исправлен (внук
> рождается после assign — как в проде: ragent порождает детей через
> ~12 с после запуска, [Ф] Б2 T-09 это и мерил). `spawn_server`
> (задача 2) обязан звать `job.assign` СРАЗУ после Popen — до детей.
...
- Produces (правки `ServersWorkspace`):
  ...
  - **Правка по финальному ревью 28.08 (воспроизведено пробой):** `Path.replace` падает `PermissionError [WinError 32]`, если журнал держит открытым процесс прошлого запуска (`dbgs`/`rmngr`, пережившие ragent, снятый из Диспетчера; `open()` в Python не даёт `FILE_SHARE_DELETE`). Ротация НЕ условие запуска: `rotate_journal` в отдельном `try/except OSError` → событие `ротация журнала не удалась (файл занят процессом прошлого запуска), записи продолжаются в тот же файл` → запуск идёт дальше. После успешного spawn — событие `порождён PID {pid}` (§12.1). Тест с реальным держателем: подставной python-ребёнок со stdout в файл журнала (kill в finally) → `start()` с `FakeServerSpawn` успешен, в файле и старое содержимое, и `запуск:`, и `порождён PID`. Долг вехи: открытие журнала в `spawn_server` через `CreateFileW` с `FILE_SHARE_DELETE` — тогда rename проходит при живых держателях.
  ...
  - Докстринги жизненного цикла переписать: серверы дочерние (§12, реверс решения 3); фраза «переживает закрытие» не должна описывать ТЕКУЩЕЕ поведение нигде в файле (историческая справка о реверсе решения — допустима).
...
**Interfaces:**
- Produces: `MainWindow.confirm_quit: Callable[[], bool] | None = None` (атрибут, ставится сборкой — как `settings_store`); `closeEvent`: ветка `close_to_tray` НЕ меняется (скрытие — не выход, серверы живут, §12.2); иначе — если `confirm_quit` задан и вернул `False` → `event.ignore()`, окно остаётся. В `app.py`: `_confirm_quit_with_servers(running_count: Callable[[], int], ask: Callable[[str], bool]) -> bool` — функция уровня модуля: `n = running_count()`; `n == 0` → `True`; иначе `ask(f"Остановить {n} серверов и выйти?")` (тексты склонения: 1 — «сервер», 2-4 — «сервера», иначе «серверов» — маленький чистый помощник `_servers_word(n)` с табличным тестом). Проводка — диалог ИНЪЕЦИРУЕТСЯ (правка 28.08 по находке исполнения: безусловный настоящий `QMessageBox` в `_build_main_window` вешал suite — существующий `test_monitor_wires_scan…` подаёт снимок с живым ragent, и teardown-закрытие окна qtbot упиралось в модальный диалог в offscreen): `_build_main_window(..., quit_dialog: Callable[[QWidget, str], bool] | None = None)`; `None` → `window.confirm_quit` остаётся `None`, гейта нет (тесты, `run_smoke`); иначе `window.confirm_quit = lambda: _confirm_quit_with_servers(servers_workspace.running_count, lambda m: quit_dialog(window, m))`. Настоящий диалог — модульная `_ask_quit_confirmation(parent: QWidget, message: str) -> bool` (`QMessageBox.question`, дефолтная кнопка No), передаётся ТОЛЬКО из `main()`; трей: `create_tray(..., on_quit=request_quit, ...)`, где `request_quit()` — тот же гейт (при отсутствии гейта — сразу) + `application.quit()`. `run_smoke`: `quit_dialog` не передаёт — `confirm_quit` не ставится. Тест-сторож против зависания: окно с работающим сервером и без `quit_dialog` закрывается без диалога (`QMessageBox.question` подменён на `raise AssertionError` — регрессия падает, а не виснет); тест, что `main()` ставит `confirm_quit` (прод-проводка не потеряна). **Правка по финальному ревью 28.08:** фикстура `_assemble` (`main()` с настоящим `_ask_quit_confirmation`) страхуется той же подменой `QMessageBox.question → raise AssertionError` — будущий тест на ней с живым сервером обязан упасть, а не повиснуть.
...
> **Заметка по итогам задачи 4 (ревью 28.08):** обязательный `server_spawn`
> сломал сборку вне файлов задачи 4, поэтому в `app.py` уже стоит ВРЕМЕННЫЙ
> шим `server_spawn=functools.partial(spawn_server, job=NullJob())` +
> `logs_dir` по формуле этой задачи (помечен комментарием «задача 7»).
> Эта задача обязана заменить шим настоящей проводкой ниже и убрать
> комментарий. `tests/ui/test_servers_view.py` на новую сигнатуру уже
> мигрирован задачей 4.
