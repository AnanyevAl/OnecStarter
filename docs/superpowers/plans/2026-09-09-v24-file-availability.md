...
**Решение заказчика 09.09.2026:** задача блокирует не Task 3, а **выпуск вехи**
(Task 12, шаг гейтов). Task 3 пишет константу `DB_FILE_NAME` с явной пометкой
`[Д] не проверено` — именно так правило проекта предписывает обращаться
с непроверенным утверждением о 1С, и запрет касается выдачи догадки за факт,
а не написания кода вообще. До замера ничего не выпускается, и цена ошибки —
одна константа.
...
**Замером T-05.17 не блокируется** (решение заказчика 09.09.2026). Константа
`DB_FILE_NAME` пишется с пометкой `[Д] не проверено` и ссылкой на замер —
догадкой за факт она при этом не выдаётся. Гейт перенесён на выпуск: Task 12
не закрывается, пока замер не превратил `[Д]` в `[Ф]`.
...
# в коде с этим именем — чтобы замер, который его подтвердит или опровергнет,
# правил одну строку.
#
# Достоверность — **из документации**: ИТС, «Приложение 7. Параметры
# командной строки запуска», ключ `/F <каталог>` — «Каталог с файлом
# 1Cv8.1CD» (скил platform-launch, reference.md). Не измерены на реальной
# машине: регистр имени на диске (для stat на Windows безразличен)
# и поведение платформы на каталоге без этого файла — замер T-05.17;
# после него пометка меняется на [Ф] со ссылкой
# на docs/research/t05-17-file-infobase-directory.md.
#
# Если замер покажет другое имя — правится эта строка. Если покажет, что
# единого имени нет вовсе, — правится правило §2 спеки, а не подгоняется код.
...
**Поколения проб (добавлено по финальному ревью ветки, 10.09.2026 — I-2).**
Перезапуск по `F5` поверх зависшей пробы означает два живых потока. Зависший
поток старой пробы, дождавшись отказа SMB, отдал бы `MISSING` **поверх** свежего
`PRESENT` новой — крестик загорался бы на восстановленном каталоге и висел
до следующего `F5`. Поэтому `AvailabilityProbe` ведёт `self._generation: int`:
`start()` увеличивает его и захватывает значение в замыкание `_run`; `report`
эмитит `probed` **только если** захваченное поколение всё ещё текущее. Лог
«закончено» для устаревшей пробы помечается словом «устарела». Тест: `spawn`,
складывающий задания в список без запуска; два `start()`; выполнить **первое**
задание после второго; убедиться, что от первого сигналов нет, от второго — есть.
...
def test_repeated_reports_cause_one_rebuild(qtbot, workspace_factory, monkeypatch):
    ...
    is state: return` — ДО строки с таймером. Если слать один и тот же путь
    пятьдесят раз, первый вызов проходит сторож и заводит таймер, остальные
    49 отсекаются и до таймера не доходят — `len(rebuilds) == 1` получается
    что с коалесингом, что без него. Ровно такой тест стоял в первой редакции
    этого плана, и мутационная проверка вехи (Task 11, мутация № 5) показала,
    что он не отличим от сломанной реализации.

    Подмена `rebuild` на ЭКЗЕМПЛЯРЕ после того, как `__init__` подключил его
    к таймеру, — приём с виду хрупкий, но измерено: PySide6 резолвит
    `self.rebuild` заново на каждый вызов сигнала, подмена действует.
    """  # noqa: RUF002
    view, _, _, _ = _view(qtbot, workspace_factory)
    rebuilds: list[int] = []
...
def test_same_key_and_state_report_is_a_no_op(qtbot, workspace_factory, monkeypatch):
    ...
    Второй отчёт с тем же `key`/`state` не должен даже завести таймер
    коалесинга, не то что вызвать пересборку.
    """  # noqa: RUF002
    view, _, _, _ = _view(qtbot, workspace_factory)
    rebuilds: list[int] = []
    original = view.rebuild
...
**Второй проход по приходу общих списков (добавлено по финальному ревью ветки,
10.09.2026 — I-1).** `common_lists_ready` — сигнал из демон-потока, его слот
`on_common` выполняется внутри `application.exec()`, то есть **после** того,
как `main` уже вызвал `start_probe()`. Значит, первая проба читает
`workspace.items()` без записей общих списков, и файловая база, присутствующая
только в `CommonInfoBases`, остаётся `UNKNOWN` до первого `F5` — при том что
общие списки чаще всего и лежат на сетевых шарах. Поэтому `on_common` после
`view.rebuild()` вызывает `start_probe()` ещё раз. Повторный `stat` локальных баз
стоит миллисекунды; поколения проб (Task 7) делают второй проход авторитетным.
Тест в `test_app.py`: эмитировать `common_lists_ready` с общим списком,
содержащим файловую базу с новым путём, и проверить, что `start_calls` двойника
вырос и второй вызов содержит её ключ пути. Тот же тест закрывает и отложенный
Minor Task 10: второй `start_probe()` обязан видеть цели, которых не было
при сборке окна.
...
строка замера T-05.17 — со статусом **по факту** на момент шага: если Task 1
ещё не проведена заказчиком, замер записывается открытым, а веха — «код готов,
ждёт замера и smoke». Записать «закрыт» без файла `docs/research/t05-17-*.md`
значило бы выдать план за факт. T-05.18 — открыт, веху не блокирует.
...
- [x] **Step 7a: Гейт достоверности — замер T-05.17 закрыт** (10.09.2026, косвенно через smoke: `docs/research/t05-17-file-infobase-directory.md`; метка `[Ф]` в коде, спеке §0 и скиле `v8i-format`)

**Блокирующий шаг** (решение заказчика 09.09.2026). Веха не выпускается, пока
замер не закрыл неизмеренное.

**Посылка решения переподтверждается** (финальное ревью ветки, 10.09.2026):
решение о гейте принималось при формулировке «в скилах имени нет — [Д]»,
а имя на деле **из документации** (ИТС, Приложение 7, `platform-launch/reference.md`).
Неизмеренными остаются регистр на диске и поведение платформы на каталоге
без файла. Заказчик выбирает: оставить гейт как есть или снизить до
«из документации достаточно для выпуска» (спека §7.4).

**Решение заказчика 10.09.2026 — снизить.** Шаг перестаёт быть блокирующим:
имя `1Cv8.1CD` — из документации ИТС, этого уровня для выпуска достаточно.
Замер T-05.17 сводится к `Get-ChildItem` на каталоге живой базы во время smoke
(шаг 8) и переводит метку в `[Ф]`; пункты 1–6 ниже выполняются по его итогу,
но выпуск их не ждёт.

Проверить и убедиться:

1. `docs/research/t05-17-file-infobase-directory.md` существует и содержит
   измеренное имя файла базы данных с датой и версией платформы;
2. значение `DB_FILE_NAME` в `src/onecstarter/services/availability.py`
   совпадает с измеренным (если нет — исправить и перепрогнать `uv run pytest`);
3. комментарий над константой переведён с «из документации» на `[Ф]`
   со ссылкой на файл замера;
4. факт возвращён в скил `v8i-format` (в `platform-launch` он уже есть);
5. таблица §0 спеки обновлена: строка про `1Cv8.1CD` — `[Ф]`;
6. два литерала `1Cv8.1CD` в докстрингах вне `availability.py` —
   `src/onecstarter/ui/dialogs/infobase.py` и `tests/ui/test_infobase_dialog.py` —
   сверены с измеренным именем (финальное ревью, M-9).

Если замер показал, что единого имени файла нет, — **остановиться**: правится
правило §2 спеки (проверять только каталог), а не подгоняется код под текст.
