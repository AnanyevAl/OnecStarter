...
# правил одну строку.
#
# Достоверность — **[Д] НЕ ПРОВЕРЕНО**: ни в скиле `v8i-format`, ни в
# `platform-launch` этого имени нет, а домысливать предметную область
# запрещено правилом проекта. Замер T-05.17 запланирован и обязателен
# до выпуска вехи; после него пометка меняется на [Ф] со ссылкой
...
в коде стоит непроверенное утверждение о 1С.
...
3. комментарий над константой переведён с `[Д] НЕ ПРОВЕРЕНО` на `[Ф]`
   со ссылкой на файл замера;
4. факт возвращён в скил `v8i-format` или `platform-launch`;
5. таблица §0 спеки обновлена: строка про `1Cv8.1CD` больше не `[Д]`.
