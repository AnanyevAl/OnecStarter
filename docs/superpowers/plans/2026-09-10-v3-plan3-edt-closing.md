...
- [x] **Step 1: Заполнить результаты Э1–Э7 дословно** (вывод команд, коды, «да/нет») — 13.09.2026, коммит `3c2f8ee`

- [x] **Step 2: Правка меток в спеке** — 13.09.2026; остались [?]: шаг 2 цепочки JDK (`-vm` из ini — не на чем проверить), несуществующий каталог `-data`, имена кодов ↔ числа
...
- [x] **Step 3: Код вслед за опровержением**
...
**Итог 13.09.2026.** Э1–Э5 код не тронули (факты подтверждены). Э6 опроверг три
допущения, все три — в одном коммите со спекой (§0-Д, §14.2, §14.3):
1. Разделитель `--project-list` — список Gogo `['a' 'b']`, а не пробел (`_LIST_SEPARATOR`
   удалён, `cli_validate_args` всегда ставит скобки).
2. Кодировка — не флаги JVM, а кодовая страница скрытой консоли: `CLI_ENCODING_ARGS`
   удалён, добавлен `wrap_console_utf8(command, comspec)` (`cmd.exe /d /v:off /c "chcp
   65001 >nul & …"`); `EdtCli(comspec=…)`, по умолчанию `%ComSpec%`; `%` в значениях
   и в командной строке — отказ `CliQuoteError` → `EdtError` до запуска.
3. Перечень проектов — не подкаталоги workspace, а реестр `.metadata\…\.projects`:
   `workspace_entries` читает реестр, `parse_project_location`/`location_blob` (домен)
   разбирают `.location` (формат [Д] Eclipse `LocalMetaArea`, совпал с байтами).

- [x] **Step 4: Таблица кодов CLI (Э7)**

В `ui/edt/console_panel.py` — `CLI_STATUS_TEXTS: dict[int, str]` (исправлено 13.09.2026:
не имена `WORKSPACE_IN_USE`, а короткие русские тексты из `help --status-codes`, они
полезнее пользователю; сопоставление имён с числами осталось [?]):
...
Тест `test_state_finished_names_known_codes` — параметризован по кодам 0, 1, 200, 202, 204;
`test_state_constants` для неизвестного кода 7 не меняется.
