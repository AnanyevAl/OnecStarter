...
> **Дополнено 20.08.2026 по находке финального ревью ветки, п. 3.** Не ошибка
> этой редакции (спека на момент Task 1 запрета не требовала — решение
> заказчика принято позже, на самом финальном ревью), а более поздняя
> находка: `Alt+F4` проходит правило допустимости этого шага (есть модификатор,
> `F4` в таблице) и **успешно регистрируется** `RegisterHotKey` — механизм
> «занятость видна отказом регистрации» (§4.3 спеки) для него не срабатывает,
> и назначив это сочетание пользователь отбирает закрытие окон у всей
> Windows. Решение заказчика 20.08.2026: запретить `Alt+F4` (модификаторы
> **ровно** `Alt`) прямо в `parse_hotkey`, не полагаясь на отказ регистрации;
> `Ctrl+Alt+F4` и `Alt+Shift+F4` система так не перехватывает — оставлены
> валидными. Правило и метка достоверности — спека §4.1, дата 20.08.2026.
...
> **Исправлено 20.08.2026 по находке финального ревью ветки, п. 4.** Рецепт
> шага реализован дословно и разошёлся со спекой §5, которая уже тогда
> требовала «вне диапазона — мягко в дефолт 10»: `max(RECENT_MIN,
> min(RECENT_MAX, value))` резало и `999`, и `-3` до границ (`50` и `0`
> соответственно), а не отдавало дефолт ни в одном случае. Ошибка в плане —
> правится и он, и код (правило проекта). Заказчик 20.08.2026 решил не
> в пользу буквы прежней спеки, а асимметрично: сверху — обрезание до
> `RECENT_MAX` (пользователь явно хотел «много», это осмысленный ввод),
> снизу (отрицательное значение) — дефолт `DEFAULT_RECENT_LIMIT`, а не
> `RECENT_MIN`/`0` (`0` — осознанный выбор «не показывать „Недавние" вовсе»,
> и молча выдавать его из битого отрицательного значения значило бы
> подменить выбор пользователя видимым изменением поведения). Итоговая
> реализация — `src/onecstarter/services/settings.py::_recent_of`:
>
> ```python
> def _recent_of(value: Any) -> int:
>     if isinstance(value, bool) or not isinstance(value, int):
...
> ```
>
> Тест `test_recent_limit_is_clamped` поправлен: кейс `(-3, 0)` стал
> `(-3, 10)`. Спека §5 приведена к тому же правилу той же датой.
...
> **Исправлено 20.08.2026 по находке финального ревью ветки, п. 2.** Абзац
> выше и код рецепта — ОШИБКА этой редакции плана: `window.close_to_tray`
> не «трей есть и им можно пользоваться», а `настройка close_to_tray AND
> трей доступен` (спека §2, `apply_close_to_tray`) — то есть УЖЕ смешивает
> пользовательскую настройку крестика с доступностью трея. Условие показа
> окна на тихом старте обязано зависеть только от доступности трея (спека
> §3.4: «трей недоступен — показать окно», без исключений по настройке
> крестика), а не от результата этого AND. С прежним условием пользователь,
> выключивший «сворачивание в трей» и включивший автозапуск, получал окно
> в лицо при каждом входе в Windows, хотя трей жив и хоткей зарегистрирован —
> скрываться было чем.
>
> Правильная реализация: `MainWindow` заводит отдельное поле
> `tray_available: bool` (не смешанное с `close_to_tray`), `_build_main_window`
> проставляет его сразу после `create_tray` (`window.tray_available = tray
> is not None`), и `main()` проверяет именно его:
> `if start_hidden and window.tray_available:`. Тест на четвёртую
> комбинацию (трей есть, `close_to_tray=False`, `start_hidden=True` →
> окно НЕ показывается) — `tests/ui/test_app.py::
> test_main_start_hidden_with_tray_and_close_to_tray_off_keeps_the_window_hidden`.
...
> **Правка 22.08.2026 (ручной прогон, шаги А3 и А6).** Эталонный блок ниже
> **предписывал дефектную строку** и был выполнен буквально: `ValueType: string`
> без `ValueData` создаёт в `Run` пустое значение, а `dontcreatekey` этому
> не мешает — флаг относится к КЛЮЧУ, а ключ `Run` есть всегда. Раздел
> «Настройки» из-за этого показывал автозапуск включённым сразу после установки.
> Действующая редакция — `ValueType: none; Flags: uninsdeletevalue`, обоснование
> в `build/installer.iss`. Блок сохранён как есть: он документирует, что именно
> план предписал, и что метка «[Из документации Inno Setup]» стояла не зря —
> проверка шагом 3 и вскрыла ошибку.
