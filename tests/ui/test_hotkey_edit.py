...
def test_cyrillic_layout_captures_the_latin_key_of_the_same_button(
    ...
    Это «принимаемая» половина пары. Отвергаемая — кириллическая буква без
    латинского позиционного эквивалента: «Ё» (`` ` ``), «Ж» (`;`), «Э» (`'`),
    **«Б» (`,`)**. Её проверяет перебор ниже, где `VK_OEM_*` попадают в ветку
    отказа; отдельный тест на «Б» удалён 20.08.2026 как строго избыточный —
    его стимул совпадал с итерацией перебора буква в букву (доказано:
    убить один, не убив другой, невозможно).

    Знание, ради которого тот тест писался, живёт здесь. Первая редакция
    этих тестов брала «Б» и утверждала, что она приходит с клавиши `B`:
    неверно, на `B` в ЙЦУКЕН стоит «И», а «Б» — на запятой. Тест доказывал
    верное утверждение событием, которого железо не порождает (находка
    финального ревью ветки, п. Б-2). Спутав два класса снова, следующий
    исполнитель решит, что правка про раскладку не работает, и «починит»
    верное поведение.

    Абсолютные `"B"` и `VK_B` здесь тоже не украшение: перебор проверяет
    согласованность с таблицей, а не саму таблицу, и согласованную
    перестановку в `_VK_BY_KEY` ловят как раз вписанные константы.
    """  # noqa: RUF002
    event = _press(
        KEY_CYRILLIC_I,
        Qt.KeyboardModifier.ControlModifier | Qt.KeyboardModifier.AltModifier,
...
def test_synthesized_event_without_native_code_still_captures(
    ...
    Несколько стимулов, а не один (разбор мутаций 20.08.2026): в первой
    редакции тест бил ровно по латинской `B` — тем же стимулом, что и соседний
    ...
    Здесь представители, а не перебор, как на драйверном пути: имена на этом
    пути даёт Qt, и перебор заставил бы тест повторять вычисление кода. Цена
    сделки измерена, чтобы она была зафиксированной, а не забытой: искажение,
    задевающее только имя вне списка параметров (скажем, `.replace("9", "")` —
    цифра `9` и `F9`), тестами не ловится. Риск принят: путь срабатывает лишь
    при `nativeVirtualKey == 0`, то есть на программно собранных событиях, —
    живая клавиатура сюда не приходит.
    """  # noqa: RUF002
    event = _press(
        key,
        Qt.KeyboardModifier.ControlModifier | Qt.KeyboardModifier.AltModifier,
...
def test_every_supported_driver_code_names_itself(application: QApplication) -> None:
    ...
    Границы перебора, чтобы их не переоценили: он проверяет согласованность
    `ui` с таблицей, а не саму таблицу. Оракулом служит та же
    `key_name_for_vk`, поэтому согласованное переименование — переставить
    в `_VK_BY_KEY` два кода вместе с именами — он не увидит. Абсолютные
    значения держат тесты `services` и константы `VK_B`/`"B"` в соседних
    тестах этого файла: удалив их как «дублирующие перебор», это свойство
    потеряют.

    Сверка `spec.vk == vk` здесь, в отличие от параметризованного теста
    выше, различающую силу имеет: проверка имени самосогласована (оракул
    и реализация читают одну таблицу), и единственным абсолютным остаётся
    round-trip через `parse_hotkey`. Проверено мутацией `_KEY_BY_VK.get(vk + 1)`,
    на которой имя и счётчик проходят, а сверка `vk` падает.
