...
@pytest.mark.parametrize(
    ...
    [
        ...
        "Ctrl+Alt+B+C",
        "мусор",
        # Долг №1 вехи: разбор ни на одном из трёх не ломается, но проверено
        # это было руками. Тест дешевле абзаца, объясняющего, почему тесту
        # тут не место.
        "+++",              # одни разделители: части пустые
        "Ctrl+Alt+Ctrl+B",  # повтор модификатора не подряд
        "Ctrl+Alt+F 5",     # пробел внутри имени клавиши, а не вокруг `+`  # noqa: RUF003
...
def test_key_name_for_vk_agrees_with_parse_hotkey() -> None:
    ...
    Диапазоны перечислены руками намеренно: взять их из самой таблицы значило
    бы сверять её с собой. Обратная сторона того же выбора (находка ревью
    ветки, п. Н-5): клавиша, добавленная в `_VK_BY_KEY` вне этих диапазонов,
    круговой охраны не получит — расширяя набор, расширить и список здесь.
    """  # noqa: RUF002
