...
## Доступность файловых баз (v2.4)

Файловую базу, чей каталог исчез с диска (отключённый съёмный носитель, удалённая
папка, недоступная сетевая шара), список показывает раньше запуска: крестик
на значке размещения и суффикс «(нет каталога)» в конце строки. Проверка каталога
и файла базы идёт в фоне при старте программы и повторяется по `F5` — без
периодической проверки по таймеру, чтобы не будить внешние диски и VPN лишний раз.
Серверные и веб-базы эта проверка не касается — их доступность решает сервер,
а не локальный диск.
...
(зелёный ▶ справа от имени — EDT запущен). «Открыть в VS Code» / «Открыть в Antigravity»
открывают каталог проекта (может лежать вне workspace). Импорт из EDT Start — вручную,
только новое. Подменю «CLI» — сборка, импорт, проверка и информация по проектам через
`1cedtcli.exe` с выводом в сворачиваемую консоль; работает только на workspace,
не открытом в EDT (иначе CLI вернёт код 202). Перечень проектов для проверки берётся
из реестра workspace (`.metadata`), а не из подкаталогов — проекты EDT обычно лежат
снаружи. Список — `%APPDATA%\OneCStarter\edt.json`, реестр EDT Start только читается.

## Установка

- **Установщик:** `OneCStarter-3.0.0-setup.exe` — per-user, права
  администратора не нужны.
- **Portable:** распаковать `OneCStarter-3.0.0-portable.zip`, запустить
  `OneCStarter.exe`.
