...
версиями платформы, `regsvr32` без повышения перещёлкивает оба вместе
([Ф] Г2, 26.08.2026, `docs/research/t07-protocol.md`: «сами GUID при смене
версии не меняются — поэтому текущая версия консоли определяется чтением
этих двух ключей без повышения»). Поэтому `registered_radmin_path` читает
`InprocServer32` (default-значение) только первого CLSID из HKLM обычным
пользователем, без UAC.
