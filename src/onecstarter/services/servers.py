...
class ServersWorkspace:
    ...
    def start(
        ...
        удалась (файл занят процессом прошлого запуска), записи
        продолжаются в тот же файл` и запуск идёт как обычно (полноценное
        ...
        try:
            ...
            self.log_event(
                profile_id,
                "ротация журнала не удалась (файл занят процессом прошлого "
                "запуска), записи продолжаются в тот же файл",
            )
        try:
