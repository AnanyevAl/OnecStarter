...
# Число в ключе — не `int`, а (длина без ведущих нулей, цифры без ведущих  # noqa: RUF003
# нулей): при равной длине лексикографика совпадает с числовой, а лимит  # noqa: RUF003
# `int()` на число разрядов (`sys.get_int_max_str_digits`, 4300) не
# достижим — имя из `.v8i` чужое, ронять пересборку дерева оно не должно.
_Number = tuple[int, str]


def _number(digits: str) -> _Number:
    stripped = digits.lstrip("0")
    return (len(stripped), stripped)


def collation_key(text: str) -> tuple[tuple[str | _Number, ...], str]:
    ...
    как в словарях (по кодам она стояла бы после «я»). Цепочки ASCII-цифр
    сравниваются как числа — по идее Проводника (`StrCmpLogicalW`), но
    не его алгоритмом: «База 2» перед «База 10», «8.3.9» перед «8.3.24»
    (решение заказчика 30.08.2026); иные цифры Юникода — текст.
    `re.split` по захватывающей группе строго чередует текст (чётные
    места) и число (нечётные), поэтому кортежи разных строк сравнимы
    поэлементно без смешения типов. Второй элемент — полный casefold —
    делает порядок «Ель»/«Ёль» и «01»/«1» устойчивым, не зависящим
    от порядка ввода.
    """  # noqa: RUF002
    folded = text.casefold()
    parts = _DIGIT_RUN.split(folded.replace("ё", "е"))  # noqa: RUF001
    natural = tuple(_number(part) if index % 2 else part for index, part in enumerate(parts))
...
def version_cell(
    ...
    *,
    ...
    установленных версий». WEB и SERVER не касается — им версия вообще
    не разрешается нами (задача 5, спека §4): для них колонка отдаёт
    свой текст и подсказку до этой проверки, независимо от того, готово
    обнаружение или нет.
    """  # noqa: RUF002
    if item.is_group:
...
# Спека v2.3, §4: выбор версии у этих баз остаётся и продолжает писать  # noqa: RUF003
# `Version` в `.v8i` (ключ читает штатный стартер), но на НАШ запуск он
# не влияет — и молчать об этом нельзя.  # noqa: RUF003
_PUBLICATION_HINT = (
...
def version_options(
    ...
    кроме установленных, и уточнять «как установлено» нечем. Дальше речь
    про случай, когда запись уже есть (`item` и `cell` переданы).
    ...
    Только для FILE (круг правок 2, item 5; круг правок 3, мелочь 1; задача 5
    вехи v2.3, спека §4): `cell.text` несёт настоящую версию только у FILE —
    у WEB это всегда «веб», у SERVER всегда «сервер» (Version на запуск ни
    той, ни другой базы не влияет), и подстановка без разбора читалась бы
    как «как установлено (веб)» или «как установлено (сервер)» — бессмыслица.
    То же для пункта запрошенной-но-не-подошедшей версии ниже: там должна
    остаться сама версия, а не слово-заглушка. Список видов, где в ячейке
    настоящая версия, свёлся к одному — фильтр переписан перечислением
    разрешённого, а не исключением, чтобы новый вид `ConnectKind` не начал
    молча попадать в эту ветку.
    """  # noqa: RUF002
    display_text = ""
