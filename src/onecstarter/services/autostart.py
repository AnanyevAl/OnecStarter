...
def is_enabled(registry: Registry) -> bool:
    ...
    Сравнения с путём текущего экземпляра по-прежнему нет: значение,
    целящее в другую копию, — это всё равно «OneCStarter стартует при
    входе», и показать «выключено» значило бы соврать о системе. Пустая
    строка — другой случай: она не запускает ничего.
