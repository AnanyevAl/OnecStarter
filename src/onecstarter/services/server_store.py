...
`range_start`/`range_end` пишутся одной строкой `"start:end"`: так
зафиксирована JSON-схема спеки §5 — диапазон портов читался бы парой
отдельных чисел ничуть не хуже, но схема файла — контракт, а не наш
произвол, и менять её форму без причины нельзя. Неизвестные ключи
