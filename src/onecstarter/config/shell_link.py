...
def _path_item(name: str, *, is_directory: bool) -> bytes:
    ...
    Граница экстраполяции ([Ф] 16.08.2026): на десктопной Windows 11
    штатный `IShellLink` наш список читает и запускает; на Windows Server
    (github-runner, en-US) — разрешает его в пустоту: все четыре поля
    ярлыка пусты. Причина не установлена (версия оболочки или кодовая
    страница), эксперимент — открытый вопрос T-04.6 в docs/tasks.md;
    на CI тест поэтому исключён маркером `requires_windows_shell`.
