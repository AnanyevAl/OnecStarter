...
СПИСОК БАЗ. Раздел не хранит своих запечённых цветов текста: перекраска
идёт через общий stylesheet (#ThemeSeg, #SettingsGroupLabel,
#SettingsBlockLabel, #SettingsNote) — а там, где QSS бессилен, кодом.
Кегль и разрядка (`letter-spacing`) заголовка группы ставятся не в QSS,
а в `CollapsibleGroup.__init__` (`ui/settings_group.py`) через `setFont`:
Qt Style Sheets `letter-spacing` не поддерживают, а он есть в утверждённом
мокапе. Цвет шеврона и линейки-разделителя группы — тоже код, а не QSS:
`set_palette(self._controller.palette)`, вызываемый и при сборке каждой
группы/блока (`_add_group`/`_add_block`), и на каждом `_sync()` — без этого
после смены темы из трея они остались бы в цветах темы, в которой раздел
строился. Шестая настройка группы «ОКНО И ЗАПУСК» — «Клиент по умолчанию»
(спека вехи «Завершение v1», §2): сегмент того же вида, что тема, только
выбор идёт в `store`, а не в `ThemeController`.

Раздел открывается свёрнутым целиком: все группы свёрнуты по умолчанию
(решение заказчика 02.09.2026, спека §1.4/§1.6). Состояние свёртки
НАМЕРЕННО не сохраняется между запусками — отдельного ключа в `settings.json`
для него нет: при каждом открытии раздела пользователь снова видит всё
свёрнутым, это не забытая настройка, а сознательное решение.
...
#
# Значение СОЗНАТЕЛЬНО ничем не сторожится тестом: тест на литерал был бы
# change-detector — падал бы при любой законной перенастройке макета и не
# ловил бы ни одного пользовательского дефекта. [Ф] замер 02.09.2026:
# в QHBoxLayout из двух элементов Qt отдаёт весь излишек единственному
# элементу с ненулевым stretch независимо от того, зафиксирован ли второй  # noqa: RUF003
# (`setFixedWidth`), поэтому снятие последнего тестами не различается.
# Ширина закрепляет соответствие утверждённому мокапу, а не поведение.  # noqa: RUF003
...
class SettingsView(QWidget):
    ...
    def _add_row(
        ...
        *,
        ...
        if wide_control:
            ...
            #
            # Ширину полю даёт именно `setFixedWidth` на `holder`: колонка
            # подписи зажата константой, и весь излишек места в строке Qt
            # обязан отдать единственному оставшемуся элементу — `control`.
            #
            # [Ф] Замер 02.09.2026 (мутационная проверка задачи 3, PySide6
            # 6.11.1): снятый `stretch=1` НЕ поменял ширину поля ни на пиксель.  # noqa: RUF003
            # Причина — не в самом флаге: в паре из двух элементов, где один
            # зажат `setFixedWidth`, весь излишек Qt отдаёт единственному
            # оставшемуся растущему элементу независимо от значения его  # noqa: RUF003
            # stretch-фактора — делить нечего, растущий один. `stretch=1`
            # оставлен как явный Qt-идиом («растущий орган несёт stretch»)
            # и страховка на случай, если в строке появится третий виджет
            # или колонка подписи перестанет быть жёстко зафиксированной —
            # тогда решать, кому достанется излишек, снова будет stretch.
            holder = QWidget()
    ...
    def _add_block(self, title: str, note: str, body: QWidget) -> None:
        ...
        Для него не годится `_add_row`: та ставит орган управления справа
        узкой колонкой, а таблице нужна ширина раздела. Регистрируется
        в `_row_notes`/`_row_controls` так же, как строки `_add_row`, —
        тесты находят блок теми же аксессорами.
