...
class ServerMonitor(QObject):
    ...
    def start(self) -> None:
        """Запустить периодический скан — но сперва сделать один немедленно.

        IMPORTANT 4a (финальное ревью ветки, §4.4): без этого карточки
        висели бы в «слепом окне» до первого тика таймера (до 5 с) — раздел
        «Серверы» показывал бы неизвестное состояние (см. `ServersView`,
        `scan_pending`) заметно дольше, чем нужно, хотя снимок можно взять
        сразу. `scan_now()` — тот же путь, что и плановый тик (флаг занятости
        общий), поэтому первый плановый тик, если он успеет случиться раньше,
        чем демон-поток вернёт результат, будет честно пропущен, а не
        продублирует скан.
        """  # noqa: RUF002
        self.scan_now()
        self._timer.start()
